...
if($profile_id != $log_id && $user_ok == true){
	$friend_check = "SELECT id FROM friends WHERE user1='$log_id' AND user2='$profile_id' AND accepted='1' OR user1='$profile_id' AND user2='$log_id' AND accepted='1' LIMIT 1";
	if(mysqli_num_rows(mysqli_query($db_conx, $friend_check)) > 0){
        $isFriend = true;
    }
	$block_check1 = "SELECT id FROM blockedusers WHERE blocker='$profile_id' AND blockee='$log_id' LIMIT 1";
	if(mysqli_num_rows(mysqli_query($db_conx, $block_check1)) > 0){
        $ownerBlockViewer = true;
    }
	$block_check2 = "SELECT id FROM blockedusers WHERE blocker='$log_id' AND blockee='$profile_id' LIMIT 1";
...
} else if($user_ok == true && $u != $log_id && $ownerBlockViewer == false){
	$friend_button = '<button onclick="friendToggle(\'friend\',\''.$profile_id.'\',\'friendBtn\')">Request As Friend</button>';
}
// LOGIC FOR BLOCK BUTTON
if($viewerBlockOwner == true){
	$block_button = '<button onclick="blockToggle(\'unblock\',\''.$profile_id.'\',\'blockBtn\')">Unblock User</button>';
} else if($user_ok == true && $profile_id != $log_id){
	$block_button = '<button onclick="blockToggle(\'block\',\''.$profile_id.'\',\'blockBtn\')">Block User</button>';
}
?>
...
<title><?php echo $u; ?> - Bell Matrimony</title>
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "friend_request_sent"){
				...
				_(elem).innerHTML = '<button onclick="friendToggle(\'friend\',\'<?php echo $profile_id; ?>\',\'friendBtn\')">Request As Friend</button>';
			} else {
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "blocked_ok"){
				elem.innerHTML = '<button onclick="blockToggle(\'unblock\',\'<?php echo $profile_id; ?>\',\'blockBtn\')">Unblock User</button>';
			} else if(ajax.responseText == "unblocked_ok"){
				elem.innerHTML = '<button onclick="blockToggle(\'block\',\'<?php echo $profile_id; ?>\',\'blockBtn\')">Block User</button>';
			} else {
				alert(ajax.responseText);
...
function printDiv() {
	...
	document.body.innerHTML ="<html><head><title></title></head><body>" + printable+ "</body>";	//Reset the page's HTML with div's HTML only
...
<li><a href="#" onclick="printDiv()" id="print_icon">Print</a><li>
</ul>
